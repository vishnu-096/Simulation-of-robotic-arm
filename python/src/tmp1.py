...
class Visualizer:
    ...
    def __init__(self, world: World, screen_size : Tuple[int, int]) -> None:
        ...
        self.start_button = pygame_gui.elements.UIButton(
            relative_rect=pygame.Rect((self.ui_button_rect[0]+100, self.ui_button_rect[0]+60), (150, self.ui_button_rect[3])),
        ...
        self.plot_button = pygame_gui.elements.UIButton(
            relative_rect=pygame.Rect((self.ui_button_rect[0]+320, self.ui_button_rect[0]+60), (150, self.ui_button_rect[3])),
            text='Plot Graphs',
            manager=self.manager
        )
    ...
    def plot_graphs(self, data_tag, robot):
        ...
        if data_tag == 'vel':
            ax = self.axes[1]
            ax.plot(time_window, robot.all_joint_velocities[0][-window_len:], '-ob', label='Theta 0_dot')
            ax.plot(time_window, robot.all_joint_velocities[1][-window_len:], '-og', label='Theta 1_dot')
            ax.plot(time_window, robot.all_joint_velocities[2][-window_len:], '-or', label='Theta 2_dot')
            ax.legend()      
            ax.set_xlabel('Time')
        self.canvas.draw()
    ...
    def display_robot_map(self, map, path):  # ld_x:List, ld_y:List, eof:Tuple(int, int))->None:
        ...
        # print(map.sensor.sensor_position)
        ...
        color = self.VIOLET
        overwrite = True
        # print(sensor_grid_x, sensor_grid_y)
        self.grid_visual.draw_cell(sensor_grid_x-1
                                   , sensor_grid_y, color, self.static_surface, overwrite)
        self.grid_visual.draw_cell(sensor_grid_x-1
                                   , sensor_grid_y-1, color, self.static_surface, overwrite)
        self.grid_visual.draw_cell(sensor_grid_x
                                   , sensor_grid_y-1, color, self.static_surface, overwrite)  
        self.grid_visual.draw_cell(sensor_grid_x
                                   , sensor_grid_y, color, self.static_surface, overwrite)
