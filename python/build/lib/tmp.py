...
class Visualizer:
    ...
    def __init__(self, world: World) -> None:
        ...
        #define static surface for grid map
        # Create a separate surface for static elements with custom dimensions
        self.static_surface = pygame.Surface(())
        self.static_surface = self.static_surface.convert((self.map_width, self.map_height))
    ...
    def draw_grid(self):
        ...
        for x in range(self.map_width-1):
            for y in range(self.map_height-1):
                ...
                pygame.draw.rect(self.static_surface, color, rect, 0)
    ...
    def display_robot_map(self, map):#ld_x:List, ld_y:List, eof:Tuple(int, int))->None:
        ...
        #update display grid with new scan regions
        for cell in map.new_landmark_grid:
            color = self.BLUE
            rect = pygame.Rect(start_x+cell[0], start_y+cell[1], blockSize, blockSize)
            pygame.draw.rect(self.screen, color, rect, 0)


        # for x in range(start_x, end_x, blockSize):
        #     for y in range(start_y, end_y, blockSize):
        #         color = self.BLACK

        #         rect = pygame.Rect(x, y, blockSize, blockSize)
        #         # print(x-start_x," :", y-start_y)

        #         if map.grid2D[y-start_y][x-start_x]==3:
        #             color=self.RED
        #             # pygame.draw.circle(self.screen, self.RED, [y-start_y, x-start_x], 5)


        #         if map.grid2D[y-start_y][x-start_x]==2:
        #             color=self.BLUE
        #             cnt+=1
        #         if map.grid2D[y-start_y][x-start_x]==1:
        #             color=self.YELLOW

        #             # y_coord.append(x-start_x)
        #             # x_coord.append(y-start_y)
